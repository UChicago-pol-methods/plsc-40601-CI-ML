...
\footnotesize
    ...
    colspec={M{1.2cm}M{1cm}M{6cm}},row{2-5}={7ex},
	 ...
	 \includegraphics[width = 1cm]{\figpath{tv-ad.png}} & \textbf A & \SetCell[r=5]{c} \vspace*{-6em} \includegraphics[width = 0.52\textwidth]{\figpath{static_estimate4.png}} \\
...
\begin{wideitemize}
\item Individuals indexed $t = 1, \dots, T$\pause
\item Treatments $W_t \in \ww$, where $|\ww| = k$\pause
\item Potential outcome $Y_t = Y_t(W_t) \in \RR$; \\ 
        vector of potential outcomes  $\Y_t = (Y_t(1), \dots, Y_t(k))$%\pause
%\item Covariates $\X_t \in \RR^p$ \pause (Also: ``context'')
\item i.i.d. $\Y_t$
\end{wideitemize}
...
%-------------------------------------------------------------------------------%
\begin{frame}{Selection metric}

Best arm:
\begin{equation*}
w^* \coloneqq \underset{w}{\argmax}\  \E[Y_t(w)].
\label{eq:best_arm}
\end{equation*}
\pause
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=\textwidth] {\figpath{static_correct4.png}} 
\end{figure}
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=.8\textwidth]{\figpath{toyTTTS.pdf}} 
\end{figure}
...
\footnotesize
    ...
    colspec={M{1.2cm}M{1cm}M{6cm}},row{2-5}={7ex},
      ...
      \SetCell[c=2]{} Advertisements &&  {Estimated \\ mean response}  \\
	 \includegraphics[width = 1cm]{\figpath{tv-ad.png}} & \textbf A & \SetCell[r=5]{c} \vspace*{-6em} \includegraphics[width = 0.52\textwidth] {\figpath{ttts_estimate4.png}} \\
	\includegraphics[width = 1cm]{\figpath{tv-ad.png}} & \textbf B &\\
	\includegraphics[width = 1cm]{\figpath{tv-ad.png}} & \textbf C &\\
	\includegraphics[width = 1cm]{\figpath{tv-ad.png}} & \textbf D &\\ \vspace*{-30em} \ \\
    \end{tblr}
...
%
%\begin{wideitemize}
%\item Individuals indexed $t = 1, \dots, T$
%\item Treatments $W_t \in \ww$, where $|\ww| = k$
%\item Potential outcome $Y_t = Y_t(W_t) \in \RR$; \\ 
%        vector of potential outcomes  $\Y_t \coloneqq (Y_t(1), \dots, Y_t(k))$
%\item \textcolor{lightgray}{Covariates $\X_t \in \RR^p$  (Also: ``context'')} 
%\item i.i.d. $(\Y_t, \textcolor{lightgray}{\X_t} )$
%\end{wideitemize}
%
%\end{frame}
%
...
\begin{wideitemize}
\item Account for some \textbf{history} at time we see individual $t$: 
\[
\HB_t=
  \begin{bmatrix}
    ...
    Y_{t-1} & W_{t-1}&  \textcolor{lightgray}{X_{[t-1]1}} & \textcolor{lightgray}{\hdots} &  \textcolor{lightgray}{X_{[t-1]p}} \\
%    \textcolor{lightgray}{?} & \textcolor{lightgray}{?} & X_{[t]1}& \hdots & X_{[t]p} \\
    \textcolor{lightgray}{\vdots} & & & & \textcolor{lightgray}{\vdots} \\
...
\begin{equation*}
\begin{split}
e_t(w)\coloneqq  \Pr\left[{W_t = w \textcolor{lightgray}{|\X_t = x}}\right]\\
...
\vspace*{-1pt}
\makebox[\linewidth]{\includegraphics[page=1,width=\paperwidth]{\figpath{TS.pdf}}}
...
\begin{tabular}{c cccc | ccc}
 &  &  & & & \multicolumn{3}{c}{$\HB_t$} \\
 $t$ & $e_t(A)$ & $e_t(B)$ & $e_t(C)$ & $e_t(D)$ &  {$Y_{t}$} & {$W_{t}$} & \textcolor{lightgray}{$X_{[t]1}$}  \\
 \hline
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=\textwidth]{\figpath{joint_correct4.png}} 
\end{figure}
...
\begin{figure}[htbp]
   \centering
   \includegraphics[width = 0.7\textwidth]{\figpath{ttts_bias4.png}} %
   \label{fig:example}
\end{figure}
\vskip -2 em
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=\textwidth] {\figpath{static_hjmax_500.png}} 
\end{figure}
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=\textwidth] {\figpath{ttts_hjmax_500.png}} 
\end{figure}
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=\textwidth]{\figpath{joint_correct4_lines.png}} 
\end{figure}
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=\textwidth] {\figpath{ttts_hjmax_1000.png}} 
\end{figure}
...
\footnotesize
    ...
    colspec={M{1.2cm}M{1cm}M{6cm}},row{2-5}={7ex},
	 ...
	 \includegraphics[width = 1cm]{\figpath{tv-ad.png}} & \textbf A & \SetCell[r=5]{c} \vspace*{-6em} \includegraphics[width = 0.52\textwidth] {\figpath{ttts_estimate_adjusted4.png}} \\
	\includegraphics[width = 1cm]{\figpath{tv-ad.png}} & \textbf B &\\
	\includegraphics[width = 1cm]{\figpath{tv-ad.png}} & \textbf C &\\
	\includegraphics[width = 1cm]{\figpath{tv-ad.png}} & \textbf D &\\ \vspace*{-30em} \ \\
    \end{tblr}
...
\begin{equation*}
\pi^* = \argmax_{\pi \in \Pi} \E[\langle \Y_t,\pi(\X_t) \rangle]
\label{eq:policy}
\end{equation*}
\pause

\begin{align*}
\textrm{H}_0 :  & \E\left[\langle \Y_t,\pi(\X_t) \rangle\right] = 0.45 \ \ \forall \pi \in \Pi \\  
\textrm{H}_A:  & \exists \ \pi \in \Pi:  \E\left[\langle \Y_t,\pi(\X_t) \rangle\right] > 0.45 \\
\end{align*}
...
\begin{equation*}
R_{cumulative}(\tilde\pi)  = \sum_{t = 1}^T \left(\E\left[\langle \Y_t,\pi^* \rangle\right] - \E\left[\langle \Y_t,\tilde \pi(\HB_t) \rangle\right] \right)
\label{eq:regret}
\end{equation*}
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=\textwidth]{\figpath{cum_benefit_joint.png}} 
\end{figure}
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=\textwidth]{\figpath{cum_benefit_joint_pval.png}} 
\end{figure}
...
%\onslide<5->{
%\begin{equation*}
%R_{simple}(\hat\pi)  = \E\left[\langle \Y_t,\pi^*(\X_t) \rangle\right] -\E\left[ \langle \Y_t,\hat \pi(\X_t) \rangle\right]
%\label{eq:regret}
%\end{equation*}
%}
...
%\tiny
	...
	\Comment{Initialize weight matrix, treatment-augmented covariate matrix, and reward vector.  
    	...
    	\For{$t = 1, \dots, T$}
		\If{$t \in \mathcal{I}_1$}
			\State Select  $w_t \sim \textrm{Uniform}(\ww)$\Comment{In first batch, assign treatment uniformly at random.}
		\Else
			\If{$t$ is the first observation in $\mathcal{I}_b$} \Comment{Update estimates of coefficient vector and variance matrix, using ridge regression with determined penalization.}
			...
			\For{$m = 1, \dots, M$}
			 ...
			 \EndFor
				\State $q_w \leftarrow \frac{1}{M} \sum_{m=1}^M 1\left\{ w = \underset{w}{\argmax} \{ x_{[1]t}^\top \tilde \theta ^{(m)}, \dots, x_{[|\ww|]t}^\top\tilde \theta^{(m)} \}  \right\}$ \Comment{ Compute TS probabilities based on observed context.}
		...
		\EndIf
		\State Select $w_t \sim \textrm{Multinom}( \e )$ \Comment{Assign treatment.}
		\State $\xi_t \leftarrow \frac{ 1 }{e_{w_t}}$ \Comment{Record inverse probability weights based on realized assignment.}
		\State $\Xi \leftarrow \diag \left(\Xi, \xi_t\right)$ \Comment{Augment weight matrix.}
		\State $\X \leftarrow [\X:x_{[w_t]t}^\top]$ \Comment{Augment covariate matrix.}
		\State $\y \leftarrow  [\y: y_t]$ \Comment{Augment reward vector.}
	\EndFor
    \end{algorithmic}
\end{algorithm}
\end{minipage}}
...
{
      \centering
      \includegraphics[width=\textwidth]{\figpath{tree_dist.png}}
      }
...
{
      \centering
      \includegraphics[width=0.95\textwidth]{\figpath{banditsCIcontextual.png}}
      }
...
{
      \centering
      \includegraphics[width=0.95\textwidth]{\figpath{banditsCIcontextual-sep.png}}
      }
...
\fontsize{5}{6}\selectfont
	...
	\Comment{Initialize priors}%, assignment vector, and reward vector. }
		...
		\If{$B = 1$}
			\State Assign treatment $w_t \leftarrow I$
		\Else
			\Repeat
				\For{$w \in \ww$}
			 	...
			 	\EndFor
			\Until{$J \neq I$}
			\State Assign treatment $w_t \leftarrow J$
		\EndIf
%		\State $\aB \leftarrow [\aB : a_t]$ \Comment{Augment assignment vector.}
%		\State $\y \leftarrow [\y : y_t]$ \Comment{Augment reward vector vector.}
			\State $(\alpha_{w_t}, \beta_{w_t}) \leftarrow (\alpha_{w_t} + \mathbbm{1}\{ y_t = 1\} + 1,  \beta_{w_t} + \mathbbm{1}\{ y_t = 0\})$.  \Comment{Update posterior distribution parameters.}
	\EndFor
    \end{algorithmic}
\end{algorithm}
 \end{minipage}
...
\fontsize{5}{6}\selectfont
	...
	\Comment{Initialize weight matrix, treatment-augmented covariate matrix, and reward vector.  
		...
		\ElsIf{$t \in \mathcal{I}_b$ for $b = 2, \dots, B$}
			\If{$t$ is the first observation in $\mathcal{I}_b$} \Comment{Update estimates of coefficient vector and variance matrix, using ridge regression with determined penalization.}
			...
			\State $e_w = \tilde{q}_w -c*u_{w} \ \ \forall w \in \ww$
		\EndIf
		\State Assign $w_t \sim \textrm{Multinom}( e_{1}, \dots,  e_{|\ww|} )$ \Comment{Assign treatment.}
		\State $\xi_t \leftarrow \frac{ 1 }{e_{w_t}}$ \Comment{Record inverse probability weights based on realized assignment.}
		\State $\Xi \leftarrow \diag \left(\Xi, \xi_t\right)$ \Comment{Augment weight matrix.}
		\State $\X \leftarrow [\X:x_{[w_t]t}^\top]$ \Comment{Augment covariate matrix.}
		\State $\y \leftarrow  [\y: y_t]$ \Comment{Augment reward vector.}
...
\begin{align*}
\E\left[ \hat{\theta}_w^{HT} \right]  %& = \E\left[ Y_t(w)\right]
& =\frac 1 T \sum_{t = 1}^T Y_t (w). 
\end{align*}
We require only independence of potential outcomes and treatment conditional on history, $Y_t(w) \independent W_t | H_t$, which is given by the experimental design.  
\begin{align*}
\E\left[ \hat{\theta}_w^{HT} \right] & = \E\left[ \frac 1 T \sum_{t=1}^T Y_{t} \frac{ \mathbbm{1} \{W_t = w \}  }{ e_{t}(w;H_t) }\right]\\
& = \frac 1 T \sum_{t=1}^T \E\left[  Y_{t} \frac{ \mathbbm{1} \{W_t = w \}  }{ e_{t}(w;H_t) }\right].
\intertext{Considering period $t$, by the Law of Iterated Expectations,}
\E\left[  Y_{t} \frac{ \mathbbm{1} \{W_t = w \}  }{ e_{t}(w;H_t) } \right]  & = \E\left[ \E\left[ Y_{t} \frac{ \mathbbm{1} \{W_t = w \}  }{ e_{t}(w;H_t) } \biggr\rvert H_t\right] \right] 
\end{align*}
Taking the interior term, $ \E\left[ Y_{t} \frac{ \mathbbm{1} \{W_t = w \}  }{ e_{t}(w;H_t) } \biggr\rvert H_t\right]$, 
by definition, 
\vskip 1 cm
\normalsize
...
\tiny
\begin{align*}
&  = \E\left[  \frac{ Y_{t} \mathbbm{1} \{W_t = w \}  }{\Pr[ W_t = w | H_t]  } \biggr\rvert H_t \right]
\intertext{By the potential outcomes model,}
&  = \E\left[ Y_{t}(w) \times \frac{ \mathbbm{1} \{W_t = w \}  }{\Pr[ W_t = w | H_t]  } \biggr\rvert H_t \right]
\intertext{And because $Y_t \independent W_t | H_t$, }
&  = \E\left[   Y_{t}(w) | H_t\right] \times \E\left[ \frac{\mathbbm{1} \{W_t = w \}  }{\Pr[ W_t = w | H_t]  } \biggr\rvert H_t \right]\\
&  = \E\left[   Y_{t}(w) | H_t\right] \times  \frac{ \E\left[ \mathbbm{1} \{W_t = w \} | H_t\right] }{\Pr[ W_t = w | H_t]  } \\
&  = \E\left[   Y_{t}(w) | H_t\right] \times  \frac{ \Pr[ W_t = w | H_t] }{\Pr[ W_t = w | H_t]  } \\
&  = \E\left[   Y_{t}(w) | H_t\right]. 
\intertext{Then returning to the Law of Iterated Expectations from above,}
 \E\left[  Y_{t} \frac{ \mathbbm{1} \{W_t = w \}  }{ e_{t}(w;H_t) } \right] &   = \E\left[ \E\left[ Y_t(w) | H_t\right] \right]  = \E\left[ Y_t(w)\right]. 
\end{align*}
\end{column}
\end{columns}
...
\only<7>{ \small
\begin{align*}
\phantom{\E\left[ \hat{\theta}_w \right] \& = \sum_{ h } \hat{\theta}_w | h  \cdot P\left( h \right)}  \\
...
\only<8-9>{ \small
\begin{align*}
\E\left[ \hat{\theta}_w \right] & = \sum_{ h } \hat{\theta}_w | h  \cdot \Pr [h]  \\
& \approx 0.458
\end{align*} }
\uncover<9>{ ($H_t$ is the history of treatment and outcomes up to time $t$.) }
...
\item Pr[h] is the probability that we ended up in that history--note that it's not equal! This is because of the unequal sampling probability, conditional on observed response
\end{wideitemize}
}
...
\vspace{10pt}
\hspace{5pt}
%For treatment $W_t$, define $e_t(w;H_t) =  \Pr[W_t = w|H_t].$\pause
\begin{align*}
 \hat{\theta}_w^{HT} & =\frac 1 T \sum_{t=1}^T Y_{t} \frac{ \mathbbm{1} \{W_t = w \}  }{e_{t}(w;H_t) }
\end{align*}
\hspace{5pt} \pause
\vspace{10pt}
\begin{wideitemize}
\item (Conditional on algorithm) $e_t(w;H_t) > 0,\forall  t, w$.\\ \pause
\item Finite-sample unbiased \citep{bowden2017unbiased} \hyperlink{htbias}{\beamerbutton{Finite-sample unbiasedness}}\pause
...
In our example\dots
\begin{align*}
 \hat{\theta}_A^{HT} & =\frac 1 3 \left[  1 \times \frac{ \mathbbm{1} \{W_t = A \}  }{1/2 } + 0 \times \frac{ \mathbbm{1} \{W_t = A \}  }{5/6 } \right]\\
 & = \frac 2 3
\end{align*}
...
\onslide<3->
\begin{align*}
\textcolor{Contrast2}{\E\left[ \hat{\theta}^{HT}_w \right]} & \textcolor{Contrast2}{ = \sum_{ h } \hat{\theta}^{HT}_w | h  \cdot \Pr[ h ]}  \\
& \textcolor{Contrast2}{= 0.5}
\end{align*}
%($H_t$ is the history of treatment and outcomes up to time $t$.) \\
\vfill
...
\begin{overprint}
\[
    \hat{\theta}_w^{DR} =\frac 1 T  \sum_{t=1}^T \only<2>{\textcolor{Contrast1}}{\underbrace{\hat{\E}[Y_t | W_t = w, X_t]}_\text{estimated outcome model}}+\frac{1}{T} \sum_{t=1}^T  \underbrace{\frac{ \mathbbm{1} \{W_t = w \} \left( Y_t - \only<2>{\textcolor{Contrast1}}{\hat{\E}[Y_t | W_t = w, X_t]} \right) }{ \only<3>{\textcolor{Contrast2}}{ e_t(w;H_t)} }}_\text{residual bias correction}
\]
...
%\item \texttt{lim[T] T*Var[Q-hat[w]] = lim[T] 1/T sum[t to T] (1/e[t][w] - 1) * (mu-hat[t] - Q[w])\^2 + 1/e[t][w] * Var(Y[w])}\\
\item \textcolor{Contrast2}{So as long as your \texttt{mu-hat[t]} model is sensible, you can decrease the first term in the asymptotic variance.}
\end{wideitemize}
}

%-------------------------------------------------------------------------------%

\begin{frame}{Off-policy evaluation}
\small

The doubly robust (DR) estimator considered here takes the form
\[
    \hat Q_T^{DR}(\pi)
    := \frac{1}{T}\sum_{t=1}^T \hat\Gamma_t(X_t,\pi),
\]
where the objects being averaged are doubly robust scores,
\[
    \hat\Gamma_t(X_t,\pi)
    := \sum_{w \in \mathcal{W}} \pi(X_t,w)
    \left[
        \hat\mu_t(X_t,w)
        + \frac{\mathbbm{1}\{W_t = w\}}{e_t(w;H_t)}
          \left(Y_t - \hat\mu_t(X_t,w)\right)
    \right].
\]
