...
save_grviz_plot <- function(widget, stem, asset_dir, width = 900, height = 620) {
  svg_path <- file.path(asset_dir, paste0(stem, ".svg"))
  png_path <- file.path(asset_dir, paste0(stem, ".png"))
  html_path <- file.path(tempdir(), paste0(stem, ".html"))
  dep_path <- file.path(tempdir(), paste0(stem, "_files"))
  chrome_candidates <- c(
  ...
  chrome_path <- chrome_candidates[nzchar(chrome_candidates) & file.exists(chrome_candidates)][1]

  if (length(chrome_path) == 0 || is.na(chrome_path)) {
    stop("Could not find a Chrome/Chromium binary for headless tree-plot screenshots.")
  }

  svg_text <- DiagrammeRsvg::export_svg(widget)
  writeLines(svg_text, svg_path, useBytes = TRUE)

  htmlwidgets::saveWidget(widget, html_path, selfcontained = FALSE, title = stem)
  on.exit(unlink(c(html_path, dep_path), recursive = TRUE, force = TRUE), add = TRUE)
  ...
  system2(
    ...
    args = c(
      ...
      paste0("--screenshot=", normalizePath(png_path, mustWork = FALSE)),
      paste0("--window-size=", width, ",", height),
      paste0("file://", normalizePath(html_path, mustWork = TRUE))
    ),
    stdout = TRUE,
    stderr = TRUE
  )

  invisible(list(svg = svg_path, png = png_path))
}
@
...
reg_tree_plot <- plot(reg_tree)
save_grviz_plot(reg_tree_plot, "regression_tree_plot", asset_dir, width = 1500, height = 720)
@
...
@
<<echo=FALSE, eval = TRUE>>=
causal_tree_plot <- plot(causal_tree)
save_grviz_plot(causal_tree_plot, "causal_tree_plot", asset_dir)
@
