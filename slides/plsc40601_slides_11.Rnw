...
Link to \href{https://github.com/UChicago-pol-methods/plsc-40601-CI-ML/blob/main/syllabus_PLSC40601-2026.pdf}{syllabus}.
...
\LARGE \textcolor{white}{Estimation.}

\end{transitionframe}
%-------------------------------------------------------------------------------%

\begin{frame}{Estimation after identification}

\begin{wideitemize}
\item Once the estimand and identification assumptions are in place, we still need an estimator. \pause
\item For binary treatment, two recurring ingredients are:
\[
m_w(x)=\E[Y_i \mid W_i=w, X_i=x], \qquad
e(x)=\Pr[W_i=1 \mid X_i=x].
\] \pause
\item Outcome-model standardization averages predictions over the target \(X\)-distribution. \pause
\item Weighting reweights observed outcomes by the inverse treatment propensity. \pause
\item Doubly robust estimators combine both ideas.
\end{wideitemize}
...
\begin{wideitemize}
\item This is just a preview for later weeks.
\item The key intuition is: once we define the estimand, we still need an estimator that uses observed data plus assumptions.
\item Standardization and weighting are the two main ingredients to flag early.
\end{wideitemize}

}

%-------------------------------------------------------------------------------%
\begin{frame}[shrink=10]{Doubly robust estimator}
...
+
\frac{W_i\left(Y_i-\widehat{m}_1(X_i)\right)}{\widehat{e}(X_i)}
-
\frac{(1-W_i)\left(Y_i-\widehat{m}_0(X_i)\right)}{1-\widehat{e}(X_i)}
\right]
\]
\pause

\begin{wideitemize}
\item Combines regression adjustment and inverse-propensity weighting. \pause
\item ``Doubly robust'' means consistency if either the outcome model or the propensity model is correctly specified. \pause
\item If both are well estimated, the estimator is often more stable and efficient. \pause
\item Still need overlap and pre-treatment covariates; post-treatment controls remain a bad idea. \pause
\item Preview: this logic returns in Week 6 for policy evaluation and in Week 7 for DR, DML, and TMLE.
\end{wideitemize}
...
\begin{wideitemize}
\item The first term is outcome-model standardization.
\item The second and third terms correct remaining imbalance using the estimated propensity score.
\item The point here is not proof; it is to give them the shape of the estimator early.
\end{wideitemize}
